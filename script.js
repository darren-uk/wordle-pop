...
function initBoard() {
	...
	// reset hidden keys on keyboard
	let checkVisibility = document.getElementsByClassName("hidden-button");
	while (checkVisibility.length)
		checkVisibility[0].classList.remove("hidden-button");
}
...
function statsPanel() {
	...
	//print statspanel
	menuContainer.innerHTML = statLayout;
...
function shadeKeyBoard(letter, color) {
	...
	function burstAnimation() {
		let selectedButtonId = "#" + letter;
		let selectedButton = document.querySelector(selectedButtonId);
		let rect = selectedButton.getBoundingClientRect();
		let buttonX = Math.floor(rect.x) + 10;
		let buttonY = Math.floor(rect.y) + 20;
	...
	for (const elem of document.getElementsByClassName("keyboard-button")) {
		if (elem.textContent === letter) {
			...
			if (color == green || color == yellow) {
				...
				elem.classList.add("hidden-button");
				burstAnimation();
			}
			break;
		}
